...
function renderUser(assigned_id, position) {
  ...
  */
  ...
  function renderTransitions(comment) {
    if(Bkg.DEBUG) console.log("Notification comment...:" + JSON.stringify(comment));
    var outerHTML = "";
  ...
  function renderIcon(type) {
    if(Bkg.DEBUG) console.log("renderIcon::Notification type:" + type);
    $img = $("<i>");
  ...
  function renderUpdatedAt(comment) {
    if(Bkg.DEBUG) console.log("renderUpdatedAt::Notification updated at:" + comment.updated_at);
    $img = $("<label>");
  ...
  Notification.render = function () {
    this.$el
      ...
      .html(Template('notification')({
        ...
        notificationType: this.model.notificationType()
      }));
  ...
  window.Views.Notification = Backbone.View.extend(Notification);
